...
div(class=" w-full  flex flex-col gap-y-8  ")
    ...
    div(class="flex flex-col  p-6 w-full h-full rounded-xl bg-white border-[#e4eaf1] border-[1px]  shadow-sm gap-y-6")
        h1(class="text-xl font-bold") Incoming Quotes Requests
        div(class="w-full h-auto flex flex-wrap gap-x-4 gap-y-4")
            
            
            div(v-for="quote in quotes" class="flex  w-full  gap-y-4  h-auto bg-white  rounded-xl shadow-sm py-6 px-7 gap-x-6 border-[#e4eaf1] border-[1px] ")
                div(class="flex flex-col w-full gap-y-4  h-auto bg-white  ")
                    ...
                    div(class="flex justify-start items-start gap-x-8")
                        div(class="flex items-center")
                            ...
                            p(class=" text-lg text-[#5d6f86]  ml-2") {{quote.start_day}}
                        div(class="flex items-center")
                            TimeslotsIcon(class="w-5 h-5 ")
                            p(class=" text-lg text-[#5d6f86]  ml-2") 12:24
                        div(class="flex items-center")
                            CalendarIcon(class="w-5 h-5 ")
                            p(class=" text-lg text-[#5d6f86]  ml-2") {{ quote.service_type }} cleaning
                        div(class="flex  items-center")
                            AreaIcon(class="w-5 h-5 ")
                            p(class=" text-lg text-[#5d6f86]  ml-2") {{quote.approx_sqm}} sq m
...
const quotes = ref(await getQuotes(userStore.getToken));
console.log(quotes);
</script>
